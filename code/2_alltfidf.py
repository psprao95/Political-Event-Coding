...
class alltfidf:
    ...
    def find_tf(self, text1, word):  # finds the term frequency of a word in a text, uses the 'find_count' method
        ...
        for j in range(lenj):
            if text1[j] == word:
                fre1 = fre1 + 1
        tf = math.log(
            1 + fre1)  # check out the other methods of finding TF, from Wikipedia. I used the logarithmic method.
        return tf
    ...
    def cluster_to_characteristic(self, infile,
                                  outfile):  # this function prints the tfidf scores of all words in a file.
